...
- **`/metaprompt`** — turns vague asks into structured execution plans with phases, success criteria, and edge cases. The BEFORE/AFTER above is the demo.
- **`/orchestrate` + `/team`** — multi-agent teams **created on the fly per task**. Analyzes the context, picks the right agents, runs them in parallel, synthesizes the results. Each agent is a markdown file you read, modify, and version-control. No vendor lock-in.
- **`/economy`** — cuts token spend mid-session when context starts swelling. One command, immediate effect.
- **`/resume`** — restores full context after `/compact` (or an accidental tab-close at 2am) via a session bridge journal that Claude itself writes during work. Pick up exactly where you left off.
- **Memory that captures belief drift** — when your agent changes its mind about something, the system notices it instead of silently overwriting. Useful when you are debugging "why does it suddenly think X" and need the trail.
- **Self-learning loop** — corrections you give Claude get auto-captured, so the same mistake does not recur next session.

Local-first by design: flat markdown + SQLite. No SaaS, no API key required, no server to host.
...
**6. Self-learning protocol (3 layers).** Captures explicit corrections, detects rhetorical-question / past-behavior / expectation-violation signals, and silently maintains a user-model that adapts communication style without compromising substance.
- Enforced via: `CLAUDE.md.template` lines 45-163 — Layer 1 (auto-capture on user correction or non-obvious discovery), Layer 2 (correction-pattern detection from rhetorical questions, past-behavior refs, expectation violations, emphasis escalation), Layer 3 (user-model observation with quality floor — adapts style, never lowers substance).
...
| `/diagnose-bound` | Mandatory persona-bound root-cause: cache → /persona-research if absent → context7 docs refresh → THEN diagnose. Refuses if precondition skipped. |
